$(function(){
	var query = $('.screening ul li'),
		...
		$('.click_search').click(function(){ //点击确认触发ajax事件并将面板隐藏
			if($('#searchvalue').val() != ''  ){
				aj();
			}
			$(querys).hide();
		...
		$('.click_aj').hammer().on("tap pan",function(){
				$(this).click();
			});
		...
		function aj(){
			...
			if($('#searchvalue').val()  != ''){
				data2["search"] = $('#searchvalue').val();
			}
			//alert($('#searchvalue').val());
			data = $.extend({}, data1,data2);
			// data = JSON.stringify($.extend({}, data1,data2)); //合并两段数据并格式化为json
			$.ajax({
				url: 'queryajax.php',
				type: 'post',
				data: data,
        	...
        	$('.demo-content').css('visibility','visible');
					...
					for(var i = 0;i < data.length;i ++){
						...
						if(data[i].type=="计划开工项目"){
							types = "type_j";
						}else{
							types = "type_x";
						}
				 		var hts = "<li> <div class='list-group'> <a class='list-group-item' href='project.php?id="+data[i].id+"'>"+data[i].name+"</a> <a class='list-group-item lastchild' href='#2'>	<span class='statuss'>"+data[i].status+"</span><span class='"+types+"'> <i class='glyphicon glyphicon-th-list'></i>"+data[i].type+"</span> <span class='statue'> <i class='glyphicon glyphicon-map-marker'></i>"+data[i].area+"</span> </a> </div> </li>";
						$(".testul").append(hts);
					}
